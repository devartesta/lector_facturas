...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upload Adobe invoices to Google Drive and register them in Postgres.")
    ...
    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            for invoice_pdf in invoice_files:
                parsed = parse_adobe_pdf(invoice_pdf)
    ...
    print(f"Uploaded {len(uploaded)} Adobe invoices to Google Drive.")
...
def discover_invoice_pdfs() -> list[Path]:
    january = FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202601" / "Gastos" / "Proveedores" / "adobe"
    february = FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202602" / "Gastos" / "Proveedores" / "adobe"
    files = list(january.glob("*.pdf")) + list(january.glob("*.PDF")) + list(february.glob("*.pdf")) + list(february.glob("*.PDF"))
...
def upsert_document_row(cursor, *, supplier_id: str | None, parsed, windows_path: str, drive_url: str, drive_file_id: str, local_source_file: str) -> None:
    ...
    payload = (
        ...
        "SL", windows_path, drive_url, None, parsed.sender_email, parsed.original_filename, "",
        ...
        parsed.original_filename, parsed.parser_name, parsed.parser_confidence, extracted_raw, "Imported from historical Adobe examples by parser.",
