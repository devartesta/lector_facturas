...
def main() -> int:
    ...
    provider = get_provider(COMPANY, "JONDO")
    uploaded = 0
    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            supplier_id = lookup_supplier_id(cur, COMPANY_CODE, "JONDO")
            for pdf_path in build_file_list():
                parsed = parse_jondo_pdf(pdf_path)
                ...
                upsert_document_row(cur, supplier_id=supplier_id, parsed=parsed, windows_path=windows_path, drive_url=str(drive_file.get('webViewLink', '')), drive_file_id=str(drive_file.get('id', '')), local_source_file=str(pdf_path))
                uploaded += 1
        conn.commit()
    print(f"Uploaded {uploaded} JONDO invoices.")
    return 0


def build_file_list() -> list[Path]:
    base = FINANCE_ROOT / "Artesta Stores (UK) Ltd" / "2026" / "1Q"
    return sorted((base / "202601_UK" / "Operating Expenses" / "202601_jondogo").glob("*.pdf")) + sorted((base / "202602_uk" / "Operating Expenses" / "202602_jondogo").glob("*.pdf"))


def lookup_supplier_id(cursor, company_code: str, supplier_code: str) -> str | None:
    row = cursor.execute(f"SELECT id FROM {SCHEMA_NAME}.suppliers WHERE company_code = %s AND supplier_code = %s", (company_code, supplier_code)).fetchone()
...
def upsert_document_row(cursor, *, supplier_id: str | None, parsed, windows_path: str, drive_url: str, drive_file_id: str, local_source_file: str) -> None:
    cursor.execute(f"SELECT id FROM {SCHEMA_NAME}.documents WHERE company_code = %s AND supplier_code = %s AND invoice_number = %s", (COMPANY_CODE, parsed.supplier_code, parsed.invoice_number))
    ...
    payload = (
        ...
        parsed.original_filename, parsed.parser_name, parsed.parser_confidence, extracted_raw, "Imported JONDO invoice.",
