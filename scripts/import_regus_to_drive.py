...
def main() -> int:
    ...
    provider = get_provider(COMPANY, "REGUS")
    pdf_path = build_file_path()
    parsed = parse_regus_pdf(pdf_path)

    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            supplier_id = lookup_supplier_id(cur, COMPANY_CODE, "REGUS")
        ...
        conn.commit()
    print(f"Uploaded REGUS invoice {parsed.invoice_number}.")
    return 0


def build_file_path() -> Path:
    return FINANCE_ROOT / "Artesta Inc" / "2026" / "1Q" / "202601" / "Operating Expenses" / "regus" / "Invoice(3313-43718).pdf"
...
def upsert_document_row(cursor, *, supplier_id: str | None, parsed, windows_path: str, drive_url: str, drive_file_id: str, local_source_file: str) -> None:
    ...
    payload = (
        ...
        parsed.original_filename, parsed.parser_name, parsed.parser_confidence, extracted_raw, "Imported REGUS invoice.",
