...
from lector_facturas.parsers.proco import parse_proco_bundle
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upload PROCO invoices and detail workbooks to Google Drive and register them in Postgres.")
    ...
    bundles = discover_proco_bundles()
    uploaded: list[tuple[str, str, str]] = []
    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            for bundle in bundles:
                parsed = parse_proco_bundle(bundle["invoice_pdf"], bundle["detail_file"])
                ...
                detail_filename = f"{base_filename}_DETAIL.xlsx"
                ...
                detail_drive = client.ensure_file(
                    ...
                    parent_id=parent_id,
                    content=bundle["detail_file"].read_bytes(),
                    mime_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                )
                detail_files = [
                    {
                        "kind": "DETAIL",
                        "original_filename": bundle["detail_file"].name,
                        "stored_filename": detail_filename,
                ...
                cur.execute(
                    ...
                    """,
                    ("LTD", parsed.supplier_code, parsed.invoice_number),
                )
                upsert_document_row(
                    cur,
                    ...
                    net_amount=parsed.manufacturing_net_amount,
                    vat_amount=parsed.manufacturing_vat_amount,
                    gross_amount=parsed.manufacturing_gross_amount,
                    review_notes="Imported from historical PROCO examples by parser. Manufacturing split.",
                )
                upsert_document_row(
                    cur,
                    supplier_id=supplier_id,
                    ...
                    net_amount=parsed.logistics_net_amount,
                    vat_amount=parsed.logistics_vat_amount,
                    gross_amount=parsed.logistics_gross_amount,
                    review_notes="Imported from historical PROCO examples by parser. Logistics split.",
                )
                uploaded.append((parsed.invoice_number, parsed.supplier_code, str(pdf_drive.get("webViewLink", ""))))
        conn.commit()

    print(f"Uploaded {len(uploaded)} PROCO invoices to Google Drive.")
    for invoice_number, supplier_code, drive_url in uploaded:
        print(f"- {supplier_code} | {invoice_number} | {drive_url}")
    return 0


def discover_proco_bundles() -> list[dict[str, Path]]:
    january = FINANCE_ROOT / "Artesta Stores (UK) Ltd" / "2026" / "1Q" / "202601_UK" / "Cost of sales" / "Proco"
    february = FINANCE_ROOT / "Artesta Stores (UK) Ltd" / "2026" / "1Q" / "202602_uk" / "Cost of sales" / "Proco"
    return [
        {
            "invoice_pdf": january / "SI6032421.pdf",
            "detail_file": january / "Artesta_January_2026.xlsx",
        },
        {
            "invoice_pdf": february / "IN6033326_001.pdf",
            "detail_file": february / "Copy of Artesta_February_2026.xlsx",
        },
    ]
...
def upsert_document_row(
    ...
    *,
    ...
    cursor.execute(
        ...
        """,
        ("LTD", parsed.supplier_code, parsed.invoice_number, division_invoice),
    )
