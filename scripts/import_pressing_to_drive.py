...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upload PRESSING invoices and detail excels to Google Drive and register them in Postgres.")
    ...
    bundles = discover_pressing_bundles()
    uploaded: list[tuple[str, str, str]] = []
    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            for bundle in bundles:
                parsed = parse_pressing_pdf(bundle["invoice_pdf"])
                ...
                for detail_suffix, detail_path in bundle["details"]:
                    detail_name = f"{parsed.supplier_code}_{parsed.invoice_date.strftime('%Y%m%d')}_{parsed.invoice_number}_{detail_suffix}.xlsx"
                ...
                payload = (
                    ...
                    "Imported from historical PRESSING examples by parser.",
                    "",
                    parsed.original_filename,
    ...
    print(f"Uploaded {len(uploaded)} PRESSING invoices to Google Drive.")
    for invoice_number, supplier_code, drive_url in uploaded:
        print(f"- {supplier_code} | {invoice_number} | {drive_url}")
    return 0


def discover_pressing_bundles() -> list[dict[str, object]]:
    january = FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202601" / "Gastos" / "Proveedores" / "Pressing"
    february = FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202602" / "Gastos" / "Proveedores" / "Pressing"
    return [
        {
            "invoice_pdf": january / "factura_26100000691-normal.pdf",
            "details": [
                ("DETAIL_CORNELLA", january / "Cornellá.xlsx"),
                ("DETAIL_SANTFELIU", january / "Sant Feliu.xlsx"),
            ],
        },
        {
            "invoice_pdf": february / "factura_26100001651-normal (2).pdf",
            "details": [
                ("DETAIL_CORNELLA", february / "OT.313362 Cornella febrer..xlsx"),
                ("DETAIL_SANTFELIU", february / "OT.313361 Sant Feliu febrer.xlsx"),
            ],
        },
    ]
