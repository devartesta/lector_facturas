...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upload Continuum invoices to Google Drive and register them in Postgres.")
    ...
    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            for invoice_pdf in invoice_files:
                parsed = parse_continuum_pdf(invoice_pdf)
    ...
    print(f"Uploaded {len(uploaded)} Continuum invoices to Google Drive.")
...
def discover_invoice_pdfs() -> list[Path]:
    january = FINANCE_ROOT / "Artesta Inc" / "2026" / "1Q" / "202601" / "Operating Expenses" / "Continuum"
    february = FINANCE_ROOT / "Artesta Inc" / "2026" / "1Q" / "202602" / "Operating Expenses" / "Continuum"
...
def upsert_document_row(cursor, *, company_code: str, supplier_id: str | None, parsed, windows_path: str, drive_url: str, drive_file_id: str, local_source_file: str) -> None:
    ...
    payload = (
        ...
        parsed.original_filename, parsed.parser_name, parsed.parser_confidence, extracted_raw, "Imported from historical Continuum examples by parser.",
