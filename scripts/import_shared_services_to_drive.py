...
FILES = [
    ("ARTESTA STORES (UK) LTD", "LTD", FINANCE_ROOT / "Artesta Stores (UK) Ltd" / "2026" / "1Q" / "202601_UK" / "Operating Expenses" / "Shared Services" / "Factura_2026-0010.pdf"),
    ("ARTESTA STORES (UK) LTD", "LTD", FINANCE_ROOT / "Artesta Stores (UK) Ltd" / "2026" / "1Q" / "202602_uk" / "Operating Expenses" / "Shared Services" / "Factura_2026-0015 (1).pdf"),
    ("ARTESTA INC", "INC", FINANCE_ROOT / "Artesta Inc" / "2026" / "1Q" / "202601" / "Operating Expenses" / "HANNUN" / "Factura_2026-0009.pdf"),
    ("ARTESTA INC", "INC", FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202602" / "Ingresos" / "INC" / "Factura_2026-0014.pdf"),
]
...
def main() -> int:
    ...
    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            cleanup_pending_shared_services(cur)
            for company, company_code, pdf_path in FILES:
                provider = get_provider(company, "SHAREDSERVICESSL")
                parsed = parse_shared_services_pdf(pdf_path)
                filename = f"{parsed.supplier_code}_{parsed.invoice_date.strftime('%Y%m%d')}_{parsed.invoice_number}.pdf"
                ...
                drive_file = client.ensure_file(name=filename, parent_id=parent_id, content=pdf_path.read_bytes(), mime_type="application/pdf")
                supplier_id = lookup_supplier_id(cur, company_code, "SHAREDSERVICESSL")
                upsert_document_row(
                    cur,
                ...
                uploaded += 1
        conn.commit()
    print(f"Uploaded {uploaded} shared services invoices.")
    return 0


def cleanup_pending_shared_services(cursor) -> None:
    cursor.execute(
        f"""DELETE FROM {SCHEMA_NAME}.documents
            WHERE supplier_code = 'SHAREDSERVICESSL'
              AND (invoice_number IS NULL OR invoice_number = '')
              AND windows_path LIKE %s""",
        ("%validation\\pending_supplier_review%",),
    )
...
def upsert_document_row(cursor, *, company_code: str, supplier_id: str | None, parsed, windows_path: str, drive_url: str, drive_file_id: str, local_source_file: str) -> None:
    ...
    payload = (
        ...
        supplier_id, parsed.supplier_code, parsed.currency_code, drive_file_id, "GOOGLE_DRIVE", parsed.document_type, "classified", "import", "", "",
        parsed.original_filename, parsed.parser_name, parsed.parser_confidence, extracted_raw, "Imported shared services invoice.",
