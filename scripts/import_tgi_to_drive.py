...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upload TGI invoices and optional monthly detail workbook to Google Drive and register them in Postgres.")
        ...
        with conn.cursor() as cur:
            for bundle in bundles:
                ...
                if bundle["detail_file"] is not None:
                    detail_name = f"{base_filename}_DETAIL{bundle['detail_file'].suffix.lower()}"
    ...
    for invoice_number, division_invoice, drive_url in uploaded:
        print(f"- {invoice_number} | {division_invoice} | {drive_url}")
    return 0


def discover_tgi_bundles() -> list[dict[str, Path | None]]:
    january = FINANCE_ROOT / "Artesta Inc" / "2026" / "1Q" / "202601" / "Operating Expenses" / "TGI"
    february = FINANCE_ROOT / "Artesta Inc" / "2026" / "1Q" / "202602" / "Cost of sales" / "tgi"
    january_detail = january / "COPYOF~1.XLS"
    return [
        {
            "invoice_pdf": january / "171197.pdf",
            "detail_file": january_detail,
        },
        {
            "invoice_pdf": january / "171198.pdf",
            "detail_file": january_detail,
        },
        {
            "invoice_pdf": february / "171420.pdf",
            "detail_file": None,
        },
        {
            "invoice_pdf": february / "171421.pdf",
            "detail_file": None,
        },
    ]
...
def upsert_document_row(
    ...
    *,
    ...
    cursor.execute(
        ...
        """,
        ("INC", parsed.supplier_code, parsed.invoice_number, parsed.division_invoice),
    )
    ...
    payload = (
        ...
        extracted_raw,
        "Imported from historical TGI examples by parser.",
        "accounting@tginc.com",
        parsed.original_filename,
