...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upload Claris invoices to Google Drive and register them in Postgres.")
    ...
    invoice_files = discover_claris_pdfs()
    uploaded: list[tuple[str, str, str]] = []
    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            for invoice_pdf in invoice_files:
                parsed = parse_claris_pdf(invoice_pdf)
    ...
    print(f"Uploaded {len(uploaded)} Claris invoices to Google Drive.")
    for supplier_code, invoice_number, drive_url in uploaded:
        print(f"- {supplier_code} | {invoice_number} | {drive_url}")
    return 0


def discover_claris_pdfs() -> list[Path]:
    january = FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202601" / "Gastos" / "Proveedores" / "CLARIS"
    february = FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202602" / "Gastos" / "Proveedores" / "claris gestio"
...
def upsert_document_row(
    ...
    *,
    ...
    payload = (
        ...
        extracted_raw,
        "Imported from historical Claris examples by parser.",
        parsed.sender_email,
        parsed.original_filename,
