...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upload REVER documents to Google Drive and register them in Postgres.")
    ...
    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            for pdf_path in build_file_list():
                parsed = parse_rever_pdf(pdf_path)
                ...
                if parsed.document_type == "invoice":
                    ...
                    destination_path = "income/other_income/supplies"
                    filename = f"{parsed.supplier_code}_SUPPLIED_{parsed.period_yyyymm}.pdf"
                windows_path = build_windows_path(COMPANY, parsed.period_yyyymm, destination_path, filename)
                ...
                uploaded.append((parsed.document_type, parsed.invoice_number, str(pdf_drive.get("webViewLink", ""))))
        conn.commit()

    print(f"Uploaded {len(uploaded)} REVER documents.")
...
def build_file_list() -> list[Path]:
    ...
    return [
        _find_file(jan, "Invoice-RVR-16471.pdf"),
        _find_file(jan, "suppliedNote (4).pdf"),
        _find_file(jan, "Invoice-RVR-16823.pdf"),
        _find_file(jan, "suppliedNote (5).pdf"),
        _find_file(feb, "Invoice-RVR-17688.pdf"),
        _find_file(feb, "suppliedNote (7).pdf"),
    ]
...
def upsert_document_row(cursor, *, supplier_id: str | None, parsed, windows_path: str, drive_url: str, drive_file_id: str, local_source_file: str) -> None:
    cursor.execute(
        f"SELECT id FROM {SCHEMA_NAME}.documents WHERE company_code = %s AND supplier_code = %s AND invoice_number = %s AND document_type = %s",
        (COMPANY_CODE, parsed.supplier_code, parsed.invoice_number, parsed.document_type),
    )
    ...
    payload = (
        ...
        parsed.original_filename, parsed.parser_name, parsed.parser_confidence, extracted_raw, f"Imported REVER {parsed.document_type}.",
