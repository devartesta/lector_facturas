...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upload UPS invoices to Google Drive and register them in Postgres.")
    ...
    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            for invoice_pdf in invoice_files:
                parsed = parse_ups_pdf(invoice_pdf)
    ...
    print(f"Uploaded {len(uploaded)} UPS invoices to Google Drive.")
...
def upsert_document_row(
    ...
    *,
    ...
    payload = (
        ...
        extracted_raw,
        "Imported from historical UPS examples by parser.",
        parsed.sender_email,
        parsed.original_filename,
