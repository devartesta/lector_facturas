...
def main() -> int:
    parser = argparse.ArgumentParser(description="Import APPHOTO invoices from review Drive folder.")
    ...
    apphoto_files = [item for item in review_files if _is_apphoto_candidate(str(item["name"]))]
    ...
    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            for item in sorted(apphoto_files, key=lambda file_item: str(file_item["name"])):
                ...
                try:
                    parsed = parse_apphoto_pdf(temp_path)
                finally:
                ...
                if document_exists(cur, company_code=COMPANY_CODE, supplier_code=parsed.supplier_code, invoice_number=parsed.invoice_number):
                    skipped.append(stored_name)
                    continue

                final_name = f"{parsed.supplier_code}_{parsed.invoice_date.strftime('%Y%m%d')}_{parsed.invoice_number}{Path(original_name).suffix.lower() or '.pdf'}"
    ...
    print(f"Imported {len(imported)} APPHOTO invoices from review folder.")
    for name in imported:
        print(f"IMPORTED\t{name}")
    for name in skipped:
        print(f"SKIPPED_ALREADY_IN_DB\t{name}")
    return 0


def _is_apphoto_candidate(stored_name: str) -> bool:
    lower = stored_name.lower()
    original = _original_name_from_review_name(stored_name).lower()
    return "facturacion-apphoto-es" in lower and original.endswith(".pdf")
...
def document_exists(cursor, *, company_code: str, supplier_code: str, invoice_number: str) -> bool:
    row = cursor.execute(
        f"""
        SELECT 1
