...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upload GLS scanned invoices to Google Drive and register them in Postgres.")
    ...
    bundles = discover_gls_bundles()
    uploaded: list[tuple[str, str]] = []
    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            for bundle in bundles:
                pdf_bytes = bundle["invoice_pdf"].read_bytes()
                ocr_text = client.ocr_pdf_to_text(name=bundle["invoice_pdf"].name, content=pdf_bytes)
                parsed = parse_gls_ocr_text(ocr_text, original_filename=bundle["invoice_pdf"].name)
                ...
                if bundle["detail_file"] is not None:
                    detail_path = bundle["detail_file"]
                    detail_name = f"{base_filename}_DETAIL{detail_path.suffix.lower()}"
                ...
                uploaded.append((parsed.invoice_number, str(pdf_drive.get("webViewLink", ""))))
        conn.commit()

    print(f"Uploaded {len(uploaded)} GLS invoices to Google Drive.")
    for invoice_number, drive_url in uploaded:
        print(f"- {invoice_number} | {drive_url}")
    return 0


def discover_gls_bundles() -> list[dict[str, Path | None]]:
    january = FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202601" / "Gastos" / "Proveedores" / "GLS"
    february = FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202602" / "Gastos" / "Proveedores" / "GLS"
    return [
        {
            "invoice_pdf": january / "Escaneo0160.pdf",
            "detail_file": january / "Copia de ARTESTA ENERO 2026.xlsx",
        },
        {
            "invoice_pdf": february / "Escaneo0011.pdf",
            "detail_file": None,
        },
    ]
...
def upsert_document_row(
    ...
    *,
    ...
    payload = (
        ...
        extracted_raw,
        "Imported from historical GLS scanned examples by OCR parser.",
        parsed.sender_email,
        parsed.original_filename,
