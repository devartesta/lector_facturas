...
class HannunImportFile:
    pdf_path: Path
    forced_period_yyyymm: str | None = None


FILES = [
    HannunImportFile(FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202601" / "Ingresos" / "HANNUN" / "Factura_2026-0006 (1).pdf"),
    HannunImportFile(FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202601" / "Ingresos" / "HANNUN" / "Factura_2026-0007 (1).pdf"),
    HannunImportFile(FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202601" / "Ingresos" / "HANNUN" / "Factura_2026-0008.pdf"),
    HannunImportFile(FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202602" / "Ingresos" / "HANNUN" / "Factura_2026-0011.pdf"),
    HannunImportFile(FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202602" / "Ingresos" / "HANNUN" / "Factura_2026-0013.pdf"),
    HannunImportFile(FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202602" / "Gastos" / "Proveedores" / "hannun" / "VTA26-007207_ARTESTASTORESL.pdf"),
    HannunImportFile(FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202603" / "Gastos" / "Proveedores" / "HANNUN" / "VTA26-010327_ARTESTASTORESL.pdf", forced_period_yyyymm="202602"),
    HannunImportFile(FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202603" / "Gastos" / "Proveedores" / "HANNUN" / "VTA26-010328_ARTESTASTORESL.pdf", forced_period_yyyymm="202602"),
    HannunImportFile(FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202603" / "Gastos" / "Proveedores" / "HANNUN" / "VTA26-010329_ARTESTASTORESL.pdf", forced_period_yyyymm="202602"),
    HannunImportFile(FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202603" / "Gastos" / "Proveedores" / "HANNUN" / "VTA26-010330_ARTESTASTORESL.pdf", forced_period_yyyymm="202602"),
]


def main() -> int:
    parser = argparse.ArgumentParser(description="Upload HANNUN invoices to Google Drive and register them in Postgres.")
    ...
    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            for import_file in FILES:
                parsed = parse_hannun_pdf(import_file.pdf_path, forced_period_yyyymm=import_file.forced_period_yyyymm)
                filename = build_filename(parsed)
                windows_path = build_windows_path(parsed.period_yyyymm, parsed.destination_path, filename)
                parent_id = ensure_drive_path(client, args.root_folder_id, windows_path)
                drive_file = client.ensure_file(
                    name=filename,
                    parent_id=parent_id,
                    content=import_file.pdf_path.read_bytes(),
                ...
                upsert_document_row(
                    ...
                    local_source_file=str(import_file.pdf_path),
                )
                uploaded += 1
        conn.commit()
    print(f"Uploaded {uploaded} HANNUN invoices.")
    return 0


def build_filename(parsed) -> str:
    safe_invoice_number = parsed.invoice_number.replace("/", "-")
    return f"{parsed.supplier_code}_{parsed.invoice_date.strftime('%Y%m%d')}_{safe_invoice_number}.pdf"


def build_windows_path(period_yyyymm: str, destination_path: str, filename: str) -> str:
    return "\\".join(
        [
            "ARTESTA - 6. Finances",
...
def ensure_drive_path(client: GoogleDriveClient, root_folder_id: str, windows_path: str) -> str:
    ...
    for folder_name in windows_path.split("\\")[1:-1]:
...
def upsert_document_row(
    ...
    *,
    ...
    payload = (
        ...
        extracted_raw,
        "Imported HANNUN invoice.",
        parsed.sender_email,
        parsed.source_subject,
