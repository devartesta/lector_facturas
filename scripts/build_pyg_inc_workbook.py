...
from lector_facturas.pyg_inc_workbook import build_pyg_inc_workbook, collect_pyg_inc_data, default_output_path
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build the INC P&G workbook from classified data.")
    ...
    repo_root = REPO_ROOT
    bundle = collect_pyg_inc_data(year=args.year, database_url=args.database_url or _read_database_url(repo_root))
    output_path = Path(args.output) if args.output else default_output_path(repo_root, args.year)
    print(build_pyg_inc_workbook(bundle, output_path))
