...
def main() -> int:
    parser = argparse.ArgumentParser(description="Import shared-services invoices from review Drive folder.")
    ...
    target_files = [
        ...
        if _original_name_from_review_name(str(item["name"])).lower().startswith("factura_2025-")
    ]
    ...
    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            for item in sorted(target_files, key=lambda file_item: str(file_item["name"])):
                ...
                try:
                    parsed = parse_shared_services_pdf(temp_path)
                finally:
                    temp_path.unlink(missing_ok=True)

                company = parsed.billed_company_name.upper()
                if company not in COMPANY_CODES:
                    skipped.append(f"{stored_name} :: unknown_company={company}")
                    continue
                provider = get_provider(company, parsed.supplier_code)
                ensure_supplier_row(cur, provider)
                supplier_id = lookup_supplier_id(cur, COMPANY_CODES[company], parsed.supplier_code)
                if document_exists(
                    cur,
    ...
    print(f"Imported {len(imported)} shared-services invoices from review folder.")
    ...
    *,
    ...
    cursor.execute(
        ...
        (
            ...
            drive_file_id,
            "GOOGLE_DRIVE",
            getattr(parsed, "document_type", "invoice"),
            "classified",
            "import",
            "",
            "",
