...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upload Spring invoices to Google Drive and register them in Postgres.")
    ...
    invoice_files = discover_spring_pdfs()
    uploaded: list[tuple[str, str]] = []
    with psycopg.connect(args.database_url) as conn:
        with conn.cursor() as cur:
            for invoice_pdf in invoice_files:
                parsed = parse_spring_pdf(invoice_pdf)
    ...
    print(f"Uploaded {len(uploaded)} Spring invoices to Google Drive.")
    for invoice_number, drive_url in uploaded:
        print(f"- {invoice_number} | {drive_url}")
    return 0


def discover_spring_pdfs() -> list[Path]:
    january = FINANCE_ROOT / "Artesta Store, S.L" / "2026" / "1Q" / "202601" / "Gastos" / "Proveedores" / "SPRING"
    files = list(january.glob("*.pdf")) + list(january.glob("*.PDF"))
...
def upsert_document_row(
    ...
    *,
    ...
    payload = (
        ...
        extracted_raw,
        "Imported from historical Spring examples by parser.",
        parsed.sender_email,
        parsed.original_filename,
