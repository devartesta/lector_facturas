...
ISSUER_COMPANY_NAME = "YUMA AI"
SUPPLIER_CODE = "YUMAAI"
SPANISH_MONTHS = {
    "enero": 1,
    "febrero": 2,
    "marzo": 3,
...
def parse_yumaai_pdf(path: Path) -> YumaInvoice:
    text = "\n".join((page.extract_text() or "") for page in PdfReader(str(path)).pages)
    return parse_yumaai_text(text, original_filename=path.name)


def parse_yumaai_text(text: str, *, original_filename: str) -> YumaInvoice:
...
def _extract(text: str, pattern: str) -> str:
    ...
    if not match:
        raise ValueError(f"Could not extract Yuma field with pattern: {pattern}")
