...
ISSUER_COMPANY_NAME = "GORGIAS INC"
SUPPLIER_CODE = "GORGIAS"
MONTHS_EN = {
...
def parse_gorgias_pdf(path: Path) -> GorgiasInvoice:
    text = "\n".join((page.extract_text() or "") for page in PdfReader(str(path)).pages)
    return parse_gorgias_text(text, original_filename=path.name)


def parse_gorgias_text(text: str, *, original_filename: str) -> GorgiasInvoice:
    normalized = text.replace("\xa0", " ").replace("\r", "")
    invoice_number = _extract(normalized, r"Invoice #.?[—-]([A-Z0-9-]+)")
    ...
    if billing_match:
        ...
        periods = re.findall(r"([A-Z][a-z]{2} [0-9]{1,2}, [0-9]{4}) to ([A-Z][a-z]{2} [0-9]{1,2}, [0-9]{4})", normalized)
        if periods:
            starts = [_parse_date(start) for start, _ in periods]
            ends = [_parse_date(end) for _, end in periods]
    ...
    gross_amount = _parse_decimal(_extract(normalized, r"Invoice Amount.?[—-]\$([0-9,]+\.[0-9]{2})"))
    net_amount = _parse_decimal(_extract(normalized, r"Total \$([0-9,]+\.[0-9]{2})"))
    return GorgiasInvoice(
...
def _extract(text: str, pattern: str) -> str:
    ...
    if not match:
        raise ValueError(f"Could not extract Gorgias field with pattern: {pattern}")
