...
DEFAULT_SALES_MARKETS = ("GB",)
DEFAULT_MANUFACTURING_LINES = ("ARTLINK", "JONDO", "PROCO")
DEFAULT_LOGISTICS_LINES = ("PROCO", "PORTCLEARANCE")
...
def default_output_path(root: Path, year: int) -> Path:
    return root / "output" / "spreadsheet" / f"pyg_ltd_{year}.xlsx"
...
def collect_pyg_ltd_data(*, year: int, database_url: str | None) -> PygLtdDataBundle:
    if not database_url or psycopg is None:
        return PygLtdDataBundle(year=year, generated_at=datetime.now(UTC), sales_rows=(), expense_rows=(), payment_fee_rows=(), provider_catalog_rows=())
    ...
    sales_rows = tuple(
        StageRow(
            ...
            entity=COMPANY_CODE,
            line_item="GB",
            detail=str(row["shipping_country_code"] or "GB"),
            amount_net=_decimal(row["amount_net"]),
            currency=str(row["payment_currency"] or "GBP"),
    ...
    for row in _filter_periodified_documents(docs):
        ...
        expense_rows.append(
            ExpenseRow(
                ...
                currency=str(row["currency_code"] or "GBP"),
                source="documents",
    ...
    payment_fee_rows = tuple(
        PaymentFeeRow(
            ...
            amount_net=abs(_decimal(row["amount_net"])),
            currency=str(row["currency_code"] or "GBP"),
...
def build_pyg_ltd_workbook(bundle: PygLtdDataBundle, output_path: Path) -> Path:
    ...
    _sheet(wb, "i-shopify-ltd", ["yyyymm", "entity", "line_item", "detail", "amount_original", "currency_original", "reporting_currency", "fx_rate", "amount_reporting", "source"], sales_sheet_rows)
    _sheet(wb, "g-expenses-ltd", ["yyyymm", "entity", "category", "subcategory", "supplier_code", "detail", "amount_original", "currency_original", "reporting_currency", "fx_rate", "amount_reporting", "source", "invoice_number", "drive_url"], expense_sheet_rows)
    _sheet(wb, "g-payment-fees-ltd", ["yyyymm", "entity", "supplier_code", "amount_original", "currency_original", "reporting_currency", "fx_rate", "amount_reporting", "source"], payment_fee_sheet_rows)
    ...
    _sheet(wb, "catalog-ltd", ["supplier_code", "supplier_name", "current_folder", "destination_path", "notes"], [[r.supplier_code, r.supplier_name, r.current_folder, r.destination_path, r.notes] for r in bundle.provider_catalog_rows])
    _main_sheet(wb, bundle)
    _count_sheet_ltd(wb, bundle)
    _add_back_links(wb)
    wb.save(output_path)
    return output_path
...
def _add_back_links(wb: Workbook) -> None:
    detail_sheets = ("i-shopify-ltd", "g-expenses-ltd", "g-payment-fees-ltd", "fx-rates", "catalog-ltd", "params")
    for title in detail_sheets:
        if title not in wb.sheetnames:
            continue
        ws = wb[title]
        ws.insert_rows(1)
        ws["A1"] = "<- Volver a P&G-LTD"
        ws["A1"]._hyperlink = Hyperlink(ref="A1", location="'P&G-LTD'!A1", display="<- Volver a P&G-LTD")
...
def _main_sheet(wb: Workbook, bundle: PygLtdDataBundle) -> None:
    ws = wb.create_sheet("P&G-LTD", 0)
    ...
    *,
    ...
    for col in [get_column_letter(i) for i in range(4, 16)]:
        ...
        for row in logistics_rows:
            ws[f"{col}{row}"] = f'=SUMIFS(\'g-expenses-ltd\'!$K:$K,\'g-expenses-ltd\'!$A:$A,{col}$1,\'g-expenses-ltd\'!$D:$D,"logistics",\'g-expenses-ltd\'!$E:$E,TRIM($A{row}))'
        for row in payment_fee_rows:
            ws[f"{col}{row}"] = f'=SUMIFS(\'g-payment-fees-ltd\'!$H:$H,\'g-payment-fees-ltd\'!$A:$A,{col}$1,\'g-payment-fees-ltd\'!$C:$C,TRIM($A{row}))'
        for row in shared_service_rows:
            ws[f"{col}{row}"] = f'=SUMIFS(\'g-expenses-ltd\'!$K:$K,\'g-expenses-ltd\'!$A:$A,{col}$1,\'g-expenses-ltd\'!$D:$D,"shared_services",\'g-expenses-ltd\'!$E:$E,TRIM($A{row}))'
        for row in administration_rows:
            ws[f"{col}{row}"] = f'=SUMIFS(\'g-expenses-ltd\'!$K:$K,\'g-expenses-ltd\'!$A:$A,{col}$1,\'g-expenses-ltd\'!$D:$D,"administration",\'g-expenses-ltd\'!$E:$E,TRIM($A{row}))'
...
def _count_sheet_ltd(wb: Workbook, bundle: PygLtdDataBundle) -> None:
    """Mirror P&G-LTD structure but show invoice counts instead of amounts."""
    ...
    ws = wb.create_sheet("Nº Facturas-LTD")
...
def _apply_ltd_layout(ws, *, pos: dict[str, int], detail_rows: list[int], daily_average_rows: list[int]) -> None:
...
def _add_navigation_links(
    ...
    *,
    ...
    links = {
        pos["shopify"]: "'i-shopify-ltd'!A1",
        sales_rows[0]: "'i-shopify-ltd'!A1",
        pos["manufacturing"]: "'g-expenses-ltd'!A1",
        manufacturing_rows[0]: "'g-expenses-ltd'!A1",
        pos["logistics"]: "'g-expenses-ltd'!A1",
        logistics_rows[0]: "'g-expenses-ltd'!A1",
        pos["payment_fees"]: "'g-payment-fees-ltd'!A1",
        payment_fee_rows[0]: "'g-payment-fees-ltd'!A1",
        pos["shared_services"]: "'g-expenses-ltd'!A1",
        shared_service_rows[0]: "'g-expenses-ltd'!A1",
        pos["administration"]: "'g-expenses-ltd'!A1",
        administration_rows[0]: "'g-expenses-ltd'!A1",
        pos["technology"]: "'g-expenses-ltd'!A1",
        technology_rows[0]: "'g-expenses-ltd'!A1",
        pos["otros_gastos"]: "'g-expenses-ltd'!A1",
    }
