...
DocumentKind = Literal["invoice", "credit_note", "receipt", "provision"]
ProvisionStatus = Literal["open", "matched", "adjusted", "cancelled"]


class DocumentListItemOut(BaseModel):
    id: str
    kind: Literal["document", "provision"]
    document_type: str = "invoice"
    company_code: str
    supplier_code: str
    supplier_name: str = ""
    ...
    drive_url: str = ""
    has_pdf: bool = False
    is_periodified_root: bool = False
    periodified_children_count: int = 0
...
class DocumentDetailOut(BaseModel):
    ...
    is_periodified_root: bool = False
    periodified_parent_id: str = ""
    periodified_children_count: int = 0
    linked_provision_id: str = ""
    linked_provision_status: str = ""
...
class ManualDocumentCreateOut(BaseModel):
    document: DocumentDetailOut


class PeriodifyMonthIn(BaseModel):
    period_yyyymm: str
    gross_amount: Decimal | None = None
    net_amount: Decimal | None = None
    vat_amount: Decimal | None = None
...
class PeriodifyDocumentIn(BaseModel):
    months: list[PeriodifyMonthIn] = Field(..., min_length=2)


class PeriodifyDocumentOut(BaseModel):
    root_document_id: str
    child_document_ids: list[str] = Field(default_factory=list)


class ProvisionCreateIn(BaseModel):
    company_code: str
    supplier_code: str
    invoice_date_expected: date | None = None
    period_yyyymm: str
    currency_code: str
    gross_amount: Decimal | None = None
    net_amount: Decimal | None = None
...
class ProvisionUpdateIn(ProvisionCreateIn):
...
class ProvisionMatchIn(BaseModel):
    document_id: str


class ProvisionAdjustmentOut(BaseModel):
    id: str
    provision_id: str
    document_id: str
    company_code: str
